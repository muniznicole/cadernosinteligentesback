...
import org.acme.service.ItemCompraService;
...
public class ItemCompraResource {
    ...
    @Transactional
    public Response inserir(ItemCompraRequest itemCompraRequest) {
        return Response.status(201).entity(service.adicionar(itemCompraRequest)).build();
    }
    ...
    @RolesAllowed({"Administrador"})
    @Path("/{id}")
    public Response alterar(@PathParam("id") Long id, ItemCompraRequest novoResquest) {
